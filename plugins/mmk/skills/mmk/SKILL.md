---
name: mmk
description: Reference and idioms for writing or modifying mmkfiles — the build files used by the `mmk` CLI (a make-like build tool with a bash-native DSL). Trigger when the user is creating, editing, debugging, or asking about a file named `mmkfile` or `Mmkfile`, or asks how to do something with the `mmk` command-line tool.
paths:
  - mmkfile
  - Mmkfile
  - "**/mmkfile"
...
`mmk` is a make-like build tool with a bash-native DSL. An `mmkfile` is **annotated bash**:
